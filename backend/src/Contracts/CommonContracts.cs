...
public static class ApiResults
{
    ...
    public static IResult Unauthorized(string message = "unauthorized") =>
        Results.Json(
            new ApiEnvelope<object>(StatusCodes.Status401Unauthorized, message, null),
            statusCode: StatusCodes.Status401Unauthorized);

    public static IResult Forbidden(string message = "forbidden") =>
        Results.Json(
            new ApiEnvelope<object>(StatusCodes.Status403Forbidden, message, null),
    ...
    public static Task WriteAsync(HttpResponse response, int statusCode, string message) =>
        response.WriteAsJsonAsync(new ApiEnvelope<object>(statusCode, message, null));
}
