...
public static class MappingExtensions
{
    ...
    public static OrderItemResponse ToResponse(this OrderItem item, bool reviewed) => new(
        item.Id,
        item.ProductId,
        item.Product.Name,
        item.Product.Merchant.StoreName,
    ...
    {
        var reviewedProductIds = order.Reviews.Select(review => review.ProductId).ToHashSet();
        return new OrderSummary(
            ...
            order.ExpireAt,
            order.CreatedAt,
            order.Items.Select(item => item.ToResponse(reviewedProductIds.Contains(item.ProductId))).ToArray());
    }

    public static OrderDetail ToDetail(this Order order)
    {
        var reviewedProductIds = order.Reviews.Select(review => review.ProductId).ToHashSet();
