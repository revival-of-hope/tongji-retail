...
模型为金额、库存、数量、评分、枚举状态和布尔值设置数据库检查约束，并为以下高频查询建立组合索引：商品状态/分类、商家商品、用户订单、待支付订单过期时间、支付状态/支付时间、商品评价、工单处理人/状态。

这些约束由 `EnsureCreated` 在新数据库中创建。已有 Oracle 数据卷不会被自动迁移；模型约束更新后应创建新卷，或另行编写正式迁移脚本。
