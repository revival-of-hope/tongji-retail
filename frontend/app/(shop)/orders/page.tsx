...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      const hasExpiredPendingOrder = orders.some(
        (order) => order.status === "PendingPayment" && new Date(order.expireAt).getTime() <= Date.now(),
      )
      if (hasExpiredPendingOrder) void load()
    }, 15_000)
  ...
  const submitReview = async () => {
    ...
    try {
      await api.createReview(reviewTarget.item.productId, {
              ...
              <Card key={order.id}>
                ...
                <CardContent className="space-y-4 p-5">
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {item.storeName} · 数量 {item.quantity}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        {order.status === "Completed" &&
                          (item.reviewed ? (
                            <Badge variant="outline">已评价</Badge>
                          ) : (
                            <Button
                              size="sm"
                              variant="outline"
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    <div>
                      <p className="text-sm text-muted-foreground">
                        配送至：{order.shippingAddress}
                      </p>
                      {order.status === "PendingPayment" && (
                        <p className="text-xs text-muted-foreground">
                          支付截止：{dateTime(order.expireAt)}
                        </p>
                      )}
                      <span>
                        合计{" "}
                        <strong className="text-lg text-primary">{currency(order.totalAmount)}</strong>
                      </span>
                    </div>
                    <div className="flex gap-2">
                      {order.status === "PendingPayment" && (
                        <Button asChild>
                          <Link href={`/orders/${order.id}/pay`}>去支付</Link>
                        </Button>
                      )}
                      {(order.status === "PendingPayment" || order.status === "PendingShipment") && (
                        <AlertDialog>
                            ...
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                {order.status === "PendingShipment"
                                  ? "已支付订单取消后将恢复库存，并将支付状态标记为已退款。"
                                  : "未支付订单将直接取消。"}
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>返回</AlertDialogCancel>
                              <AlertDialogAction onClick={() => void action(order.id, "cancel")}>
                                确认取消
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                      ...
                      {order.status === "Shipped" && (
                        <Button onClick={() => void action(order.id, "complete")}>确认收货</Button>
                      )}
                    </div>
                  </div>
                </CardContent>
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{reviewTarget?.item.productName}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="review-comment">评价内容</Label>
                <Textarea
                  id="review-comment"
                  value={comment}
            ...
            <DialogFooter>
              <Button disabled={submittingReview} onClick={() => void submitReview()}>
